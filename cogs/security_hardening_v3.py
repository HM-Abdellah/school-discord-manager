...
from services.storage import enroll_student_record, get_active_academic_year, get_guild_config, reset_guild_data
...
class HardenedManagementCommands(commands.Cog):
    ...
    async def assign_student(self, interaction: discord.Interaction, student: discord.Member, level: str, stream: str) -> None:
        ...
        try:
            ...
            try:
                current_added = [role for role in student.roles if role not in original_roles and not role.managed]
                if current_added:
                    await student.remove_roles(*current_added, reason="School Manager assignment rollback")
