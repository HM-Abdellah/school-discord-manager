...
class StudentCommands(commands.Cog):
    ...
    async def assign_student(self, interaction: discord.Interaction, student: discord.Member, class_role: discord.Role) -> None:
        ...
        if not class_role.name.startswith(CLASS_ROLE_PREFIX):
            await interaction.response.send_message("❌ Ce n'est pas un rôle de classe School Discord Manager.", ephemeral=True)
            return
        ...
        if year is None:
            await interaction.response.send_message("❌ Aucune année scolaire active. Lance `/setup`.", ephemeral=True)
            return
        class_name = class_role.name.removeprefix(CLASS_ROLE_PREFIX)
        school_class = find_class(interaction.guild.id, int(year["id"]), class_name)
        if school_class is None:
            await interaction.response.send_message("❌ Cette classe n'existe pas dans l'année scolaire active.", ephemeral=True)
            return

        old_roles = [r for r in student.roles if r.name.startswith(CLASS_ROLE_PREFIX) and r != class_role]
        try:
            if old_roles:
                await student.remove_roles(*old_roles, reason="Student class transfer")
            ...
            await interaction.response.send_message("❌ Vérifie la hiérarchie des rôles du bot.", ephemeral=True)
            return

        await interaction.response.send_message(
            f"✅ {student.mention} est maintenant dans **{class_name}**. Le changement est enregistré dans son historique.",
            ephemeral=True,
        )
    ...
    async def student_history(self, interaction: discord.Interaction, student: discord.Member) -> None:
        ...
        if not rows:
            await interaction.response.send_message("ℹ️ Aucun historique enregistré pour cet élève.", ephemeral=True)
            return
        lines = [f"## 📚 Historique de {student.display_name}", ""]
        for row in rows:
            end = row["end_date"] or "présent"
            lines.append(f"• **{row['academic_year']}** — {row['stream_name']} / {row['class_name']} — {row['start_date']} → {end} ({row['status']})")
        await interaction.response.send_message("\n".join(lines), ephemeral=True)

    @app_commands.command(name="leavschool", description="Marquer un élève comme ayant quitté l'établissement.")
    @app_commands.describe(student="Élève qui quitte l'établissement")
    @app_commands.checks.has_permissions(administrator=True)
    async def leave_school(self, interaction: discord.Interaction, student: discord.Member) -> None:
        ...
        if row is None:
            await interaction.response.send_message("❌ Cet élève n'est pas encore enregistré dans la base.", ephemeral=True)
            return
