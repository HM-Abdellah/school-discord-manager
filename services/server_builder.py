...
def _subject_channel_name(stream_code: str, subject: str) -> str:
    subject_name = _safe_name(get_subject_display_name(subject), 55)
    return f"📚-{stream_code}・{subject_name}"


def _stream_header_name(stream_name: str, stream_code: str) -> str:
    emoji = STREAM_EMOJIS.get(stream_name, "🎓")
    return f"🔹-{emoji}・{stream_code}"
...
class ServerBuilder:
    """Build a Discord hierarchy grouped by level, with streams visually grouped by headers."""
        ...
        for role_name, colour in (
            ...
            if role is None:
                ...
                if role_name == ROLE_ADMIN:
                    ...
                    for permission in (
                        "view_channel", "read_message_history", "connect", "speak", "stream", "use_application_commands"
                    ):
                        setattr(permissions, permission, True)
                else:
                    for permission in (
                        "view_channel", "send_messages", "read_message_history",
                        "create_public_threads", "send_messages_in_threads", "connect", "speak", "use_application_commands"
                    ):
                        setattr(permissions, permission, True)
                role = await self.guild.create_role(
                    name=role_name, permissions=permissions, colour=colour,
                    hoist=True, mentionable=True, reason="School manager main role"
                )
    ...
    async def _ensure_stream_role(self, level_name: str, stream_name: str) -> discord.Role:
        ...
        if role is None:
            role = await self.guild.create_role(
                ...
                mentionable=True, reason="School manager stream role"
            )
            self.stats.roles_created += 1
        return role
    ...
    async def _get_or_create_text(self, category, name, *, topic, overwrites, preserve_member_overwrites=False):
        ...
        if channel:
            effective_overwrites = dict(overwrites)
            if preserve_member_overwrites:
                for target, permission in channel.overwrites.items():
                    if isinstance(target, discord.Member):
                        effective_overwrites[target] = permission
            await channel.edit(topic=topic, overwrites=effective_overwrites, reason="School manager reconciliation")
            return channel
    ...
    async def _ensure_general_area(self, roles):
        overwrites = general_area_overwrites(
            self.guild.default_role, roles[ROLE_ADMIN], roles[ROLE_PROFESSOR], roles[ROLE_PROFESSOR_FEMALE], roles[ROLE_STUDENT]
        )
    ...
    async def _build_level(self, level, roles, voice_category):
        ...
        level_category = await self._get_or_create_category(_level_category_name(level_name), None)
        for stream in level.get("streams", []):
            ...
            stream_overwrites = stream_area_overwrites(
                ...
                roles[ROLE_PROFESSOR_FEMALE], roles[ROLE_STUDENT], stream_role
            )
            ...
            # Discord does not support nested categories. A locked header channel visually groups each stream.
            await self._get_or_create_text(
                level_category,
                _stream_header_name(stream_name, stream_code),
                topic=f"Filière {stream_code} — {stream_name}. Utiliser les salons qui suivent pour cette filière.",
                overwrites=announcement_overwrites,
            )
            await self._get_or_create_text(
                level_category, f"📌-{stream_code}・informations",
                topic=f"Informations et organisation de {stream_name} — {level_name}.",
                overwrites=announcement_overwrites,
            )
            await self._get_or_create_text(
                level_category, f"🗓️-{stream_code}・emploi-du-temps",
                topic=f"Emplois du temps de tous les groupes/classes de {stream_name} en {level_name}.",
                overwrites=announcement_overwrites,
            )
            await self._get_or_create_text(
                level_category, f"📝-{stream_code}・examens",
            ...
            for subject in subjects:
                await self._get_or_create_text(
                    ...
                    topic=(
                        ...
                        f"pour toute la filière {stream_name} ({level_name}). Seuls les enseignants affectés à cette matière peuvent publier."
                    ),
                    overwrites=subject_channel_overwrites(
                        self.guild.default_role, roles[ROLE_ADMIN], roles[ROLE_PROFESSOR],
                        roles[ROLE_PROFESSOR_FEMALE], stream_role
                    ),
