...
async def test_administrator_permission_is_not_a_management_bypass():
    ...
    guild = SimpleNamespace(owner_id=999, roles=[everyone, bot_role], default_role=everyone, me=SimpleNamespace(top_role=bot_role))
...
async def test_administration_role_is_accepted():
    ...
    bot_role = _role("Bot", 10)
    guild = SimpleNamespace(owner_id=999, roles=[everyone, admin, bot_role], default_role=everyone, me=SimpleNamespace(top_role=bot_role),)
    response = SimpleNamespace(is_done=lambda: False, send_message=_async_noop)
...
def test_hierarchy_error_identifies_low_bot_role():
    ...
    bot_role = _role("Bot", 10)
    guild = SimpleNamespace(owner_id=1, roles=[everyone, school_role, bot_role], default_role=everyone, me=SimpleNamespace(top_role=bot_role))
