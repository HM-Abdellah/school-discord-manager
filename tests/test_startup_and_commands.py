...
def test_critical_commands_have_one_source_definition_and_expected_owner():
    ...
    for command, owner in EXPECTED_RUNTIME_OWNERS.items():
        assert definitions[command] == [owner], f"{command} owner mismatch: {definitions[command]}"
...
def test_command_cogs_use_service_layer_not_other_command_cogs():
    command_cogs = (
        ...
        "cogs/year_rollback.py",
        "cogs/security_hardening_v3.py",
    )
...
def test_security_cog_owns_only_security_commands():
    commands = _command_names_in_file("cogs/security_hardening_v3.py")
    assert sorted(commands) == ["assignstudent", "assignteacher", "resetserver"]


def test_legacy_security_v2_has_no_runtime_commands():
    assert _command_names_in_file("cogs/security_hardening_v2.py") == []
    source = Path("cogs/security_hardening_v2.py").read_text(encoding="utf-8")
...
def test_legacy_resource_discovery_requires_exact_canonical_names():
    config = {
        "levels": [{"name": "Tronc Commun", "streams": [{"name": "Tronc Commun Scientifique", "abbreviation": "TCS", "subjects": ["Mathématiques"]}]}],
    ...
    guild = SimpleNamespace(
        ...
        roles=[SimpleNamespace(id=501, name="Filière - TCS", managed=False), SimpleNamespace(id=502, name="My TCS Role", managed=False)],
    )
...
def test_build_structure_does_not_require_on_demand_subject_roles():
    config = {
        "levels": [{"name": "Tronc Commun", "streams": [{"name": "Tronc Commun Scientifique", "abbreviation": "TCS", "subjects": ["Mathématiques", "Physique-Chimie"]}]}]
    }
