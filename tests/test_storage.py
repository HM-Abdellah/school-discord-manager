...
def test_duplicate_active_enrollments_are_deduplicated(tmp_path, monkeypatch):
    data_dir = tmp_path / "data"
    monkeypatch.setattr(storage, "DATA_DIR", data_dir)
    monkeypatch.setattr(storage, "DATABASE_FILE", data_dir / "school.db")
    ...
    storage.initialize_database()
    with storage._connect() as conn:
        active = conn.execute("SELECT COUNT(*) FROM enrollments WHERE student_id=? AND status='active'", (student,)).fetchone()[0]
        assert active == 1
